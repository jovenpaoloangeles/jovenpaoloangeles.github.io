...
const RAW = [
  { id: 'd1', num: '01', name: 'Scientific Computing & Research', short: 'Scientific',
    ...
    tools: [
      ...
      { id: 'matlab', name: 'MATLAB', level: 'sup', role: 'Engineering & numerical computing', icon: cdn('matlab/matlab') },
      { id: 'r', name: 'R', level: 'sup', role: 'Statistical analysis & visualization', icon: cdn('r/r') },
      ...
      { id: 'plotly', name: 'Plotly', level: 'sup', role: 'Interactive dashboards', icon: cdn('plotly/plotly') },
  ...
  { id: 'd2', num: '02', name: 'Machine Learning & Optimization', short: 'ML & Optimization',
    ...
    tools: [
      { id: 'pytorch', name: 'PyTorch', level: 'sig', role: 'Deep learning framework & tensors', icon: cdn('pytorch/pytorch'), also: ['d1', 'd3'] },
      ...
      { id: 'mlflow', name: 'MLflow', level: 'sig', role: 'End-to-end ML lifecycle management', icon: mono('ML') },
      ...
      { id: 'diffusers', name: 'Diffusers', level: 'sup', role: 'Generative image pipelines & LoRA fine-tuning', icon: mono('Df'), also: ['d3'] },
      { id: 'shap', name: 'SHAP', level: 'sup', role: 'Model interpretability & explainable AI', icon: mono('SH'), parent: 'scikitlearn', isNew: true },
      { id: 'optuna', name: 'Optuna', level: 'sup', role: 'Hyperparameter optimization framework', icon: mono('Op'), isNew: true },
    ],
  },
  { id: 'd3', num: '03', name: 'LLMs & Generative AI', short: 'LLMs & GenAI',
    ...
    tools: [
      ...
      { id: 'claude', name: 'Claude', level: 'sig', role: 'Anthropic LLM API & ecosystem', icon: lobe('claude'), isNew: true },
      ...
      { id: 'chromadb', name: 'ChromaDB', level: 'sup', role: 'Embedding database for RAG', icon: mono('Ch') },
      { id: 'claudecode', name: 'Claude Code', level: 'sup', role: 'CLI agent for Claude', icon: lobe('claude'), parent: 'claude', isNew: true },
      { id: 'bedrock', name: 'AWS Bedrock', level: 'sup', role: 'Serverless LLM orchestration & deployment', icon: lobe('bedrock'), parent: 'claude', isNew: true },
      ...
      { id: 'unsloth', name: 'Unsloth', level: 'sup', role: 'Fast LLM fine-tuning', icon: mono('Us') },
  ...
  { id: 'd4', num: '04', name: 'Web Development & Interfaces', short: 'Web & Interfaces',
    ...
    tools: [
      ...
      { id: 'n8n', name: 'n8n', level: 'sup', role: 'Workflow automation', icon: cdn('n8n/n8n') },
      { id: 'flask', name: 'Flask', level: 'chip', role: 'Lightweight web framework', icon: cdn('flask/flask') },
    ],
  },
  { id: 'd5', num: '05', name: 'Data & Infrastructure', short: 'Data & Infra',
    ...
    tools: [
      ...
      { id: 'redis', name: 'Redis', level: 'sup', role: 'In-memory data structure store & cache', icon: cdn('redis/redis'), isNew: true },
      ...
      { id: 'nginx', name: 'Nginx', level: 'sup', role: 'Reverse proxy', icon: cdn('nginx/nginx') },
      { id: 'vercel', name: 'Vercel', level: 'sup', role: 'Frontend deployment', icon: lobe('vercel') },
      { id: 'prometheus', name: 'Prometheus', level: 'chip', role: 'Metrics & monitoring', icon: cdn('prometheus/prometheus') },
      { id: 'opentelemetry', name: 'OpenTelemetry', level: 'chip', role: 'Observability framework', icon: cdn('opentelemetry/opentelemetry') },
      { id: 'openlit', name: 'OpenLit', level: 'chip', role: 'OTel-native LLM observability', icon: mono('OL'), parent: 'opentelemetry' },
      { id: 'grafana', name: 'Grafana', level: 'chip', role: 'Data visualization & monitoring dashboards', icon: cdn('grafana/grafana'), parent: 'prometheus', isNew: true },
      { id: 'jenkins', name: 'Jenkins', level: 'chip', role: 'Automation server for CI/CD', icon: cdn('jenkins/jenkins') },
    ],
  },
  { id: 'd6', num: '06', name: 'Creative Coding & Visualization', short: 'Creative',
    ...
    tools: [
      { id: 'comfyui', name: 'ComfyUI', level: 'sig', role: 'Node-based graphical interface for stable diffusion pipelines', icon: lobe('comfyui'), isNew: true },
      { id: 'p5js', name: 'p5.js', level: 'sig', role: 'Creative coding in JavaScript', icon: cdn('p5js/p5js') },
      ...
      { id: 'processing', name: 'Processing', level: 'sup', role: 'Visual arts & creative coding', icon: cdn('processing/processing') },
